...
function tpl_modifier_date_format($string, $format="%b %e, %Y", $default_date=null){
	if ($string != '') return strftime($format, tpl_make_timestamp($string));
	elseif (isset($default_date) && $default_date != '') return strftime($format, tpl_make_timestamp($default_date));
	else return;
}

if(!function_exists('tpl_make_timestamp')) {
	function tpl_make_timestamp($string) {
		if(empty($string)) $string = "now";
		$time = strtotime($string);
		if (is_numeric($time) && $time != -1) return $time;
